...
class MotionBaseModel(BaseModel):
    ...
    # new
    def build_input_dict_own(self, frame_id, prev_frame, this_frame, results_bbs):
        prev_pc = o3d.io.read_point_cloud(prev_frame)
        prev_pc = np.array(prev_pc.points)
        this_pc = o3d.io.read_point_cloud(this_frame)
        this_pc = np.array(this_pc.points)
        prev_pc = datasets.data_classes.PointCloud(prev_pc)
        this_pc = datasets.data_classes.PointCloud(this_pc)
        ref_box = results_bbs[-1]
        prev_frame_pc = points_utils.generate_subwindow(prev_pc, ref_box,
                                                        scale=self.config.bb_scale,
                                                        offset=self.config.bb_offset)
